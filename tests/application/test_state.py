...
from tests.application.fakes import FakeNodeStateStore, FakeZonePairs
...
def test_get_all_points_and_zone_state():
    ...
    zones = FakeZonePairs({"AE5": [("start_1", "end_1")]})
    zone = GetZoneState(store, zones).execute("ae5")
    assert zone.success
    assert zone.data["zone"] == "AE5"
    assert "start_1" in zone.data["nodes"]

    missing = GetZoneState(store, zones).execute("NOPE")
    assert not missing.success
